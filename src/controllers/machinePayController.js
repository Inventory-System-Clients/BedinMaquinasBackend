...
export const consultarFechamento = async (req, res) => {
  try {
    const { maquinaId, inicio, fim } = req.query;

    if (!maquinaId || !inicio || !fim) {
      return res.status(400).json({
        error: "Informe maquinaId, inicio e fim para consultar a Machine Pay.",
      });
    }

    const maquina = await buscarMaquinaMachinePay(maquinaId);
    ...
    console.error("[MachinePay] Erro ao consultar fechamento:", error);
    res.status(error.status || 502).json({
      error: error.message || "Nao foi possivel consultar a Machine Pay.",
    });
  }
};
