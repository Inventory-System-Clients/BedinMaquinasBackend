...
export const relatorioRoteiro = async (req, res) => {
  try {
    ...
    if (!lojas || lojas.length === 0) {
      return res
        .status(404)
        .json({ error: "Nenhuma loja encontrada para o roteiro" });
    }
          ...
          include: [
            {
              model: Produto,
              as: "produto",
              attributes: ["id", "nome", "codigo", "emoji", "preco"],
            },
          ],
        },
      ],
    ...
    const lojasDetalhadas = lojas.map((loja) => {
      ...
      return {
        ...
        totais: {
          ...
          produtosEntraram: movsLoja.reduce(
            (sum, m) => sum + (m.abastecidas || 0),
            0,
          ),
          movimentacoes: movsLoja.length,
          valoresEntrada: {
            fichas: movsLoja.reduce(
              (sum, m) => sum + parseFloat(m.valorEntradaFichas || 0),
              0,
            ),
            notas: movsLoja.reduce(
              (sum, m) => sum + parseFloat(m.valorEntradaNotas || 0),
              0,
            ),
            cartao: movsLoja.reduce(
              (sum, m) => sum + parseFloat(m.valorEntradaCartao || 0),
              0,
            ),
          },
  ...
  try {
    ...
    movimentacoes.forEach((mov) => {
      mov.detalhesProdutos?.forEach((mp) => {
        ...
        if (!movimentosPorProdutoMaquina[key])
          movimentosPorProdutoMaquina[key] = [];
    ...
    Object.values(movimentosPorProdutoMaquina).forEach((movs) => {
      ...
      for (let i = 0; i < movs.length; i++) {
        ...
        if (i === 0) {
          ...
          mov.estoquePre =
            estoquePos + mov.quantidadeSaiu - mov.quantidadeAbastecida;
      ...
      if (movs.length > 1) {
        ...
        const quantidadeSaiu = Math.abs(
          (movs[0].estoquePre ?? 0) - (movs[movs.length - 1].estoquePre ?? 0),
        );
        if (!produtosSairamMap[key]) {
    ...
    const totalProdutosSairam = Object.values(produtosSairamMap).reduce(
  ...
  try {
    ...
    if (maquinaIds.length > 0) {
      const movimentacoesOrdenadas = await Movimentacao.findAll({
        attributes: ["id", "maquinaId", "totalPos", "dataColeta", "createdAt"],
        where: {
    ...
    for (const maquina of maquinas) {
      ...
      const estoqueAtual = Number(ultimaMovimentacao?.totalPos || 0);
      const capacidadePadrao = Number(maquina.capacidadePadrao || 0);
      const percentualAlerta = Number(maquina.percentualAlertaEstoque || 0);
      const estoqueMinimo =
        (capacidadePadrao * percentualAlerta) / 100;
      const percentualAtual =
        capacidadePadrao > 0 ? (estoqueAtual / capacidadePadrao) * 100 : 0;

      if (estoqueAtual < estoqueMinimo) {
        const produtosMap = new Map();

        ultimaMovimentacao?.detalhesProdutos?.forEach((detalhe) => {
          if (!detalhe.produto) return;

          produtosMap.set(detalhe.produto.id, {
            id: detalhe.produto.id,
            nome: detalhe.produto.nome,
            codigo: detalhe.produto.codigo,
            emoji: detalhe.produto.emoji,
          });
        });
  ...
  try {
    ...
    const performance = await Movimentacao.findAll({
      attributes: [
        ...
        [
          literal(
            "SUM(COALESCE(valor_entrada_fichas, 0) + COALESCE(valor_entrada_notas, 0) + COALESCE(valor_entrada_cartao, 0))",
          ),
          "totalFaturamento",
        ],
...
export const relatorioImpressao = async (req, res) => {
  try {
    ...
    // Consolidar produtos que saíram usando os detalhes por produto da movimentação.
    const produtosSairamMap = {};
    const produtosCandidatosSairamMap = {};
    movimentacoes.forEach((mov) => {
      mov.detalhesProdutos?.forEach((mp) => {
        if (mp.quantidadeSaiu > 0) {
          const key = mp.produtoId;
        ...
        // Candidato para fallback: mantém identidade do produto quando existir movimentação do item.
        if (
          mp?.produto &&
          (mp.quantidadeSaiu > 0 || mp.quantidadeAbastecida > 0)
        ) {
          const key = mp.produtoId;
          if (!produtosCandidatosSairamMap[key]) {
            produtosCandidatosSairamMap[key] = {
              produto: mp.produto,
            };
          }
        }
    ...
    if (produtosSairam.length === 0 && totalSairam > 0) {
      const candidatos = Object.values(produtosCandidatosSairamMap);
      if (candidatos.length === 1) {
        produtosSairam.push({
          produto: candidatos[0].produto,
          quantidade: totalSairam,
        });
      }
    }
    ...
    const totalProdutosSairamDetalhado = produtosSairam.reduce(
      (sum, p) => sum + (p.quantidade || 0),
      0,
    );
    ...
    movimentacoes.forEach((mov) => {
      ...
      dadosPorMaquina[maquinaId].valoresEntrada.fichas += parseFloat(
        mov.valorEntradaFichas || 0,
      );
      dadosPorMaquina[maquinaId].valoresEntrada.notas += parseFloat(
        mov.valorEntradaNotas || 0,
      );
      dadosPorMaquina[maquinaId].valoresEntrada.cartao += parseFloat(
        mov.valorEntradaCartao || 0,
      );

      // Guardar última movimentação para cada máquina (mais recente)
      if (
        !dadosPorMaquina[maquinaId].ultimaMovimentacao ||
        new Date(mov.dataColeta) >
          new Date(dadosPorMaquina[maquinaId].ultimaMovimentacao.dataColeta)
      ) {
      ...
      mov.detalhesProdutos?.forEach((mp) => {
        ...
        if (
          ...
          const key = mp.produtoId;
          if (!dadosPorMaquina[maquinaId].produtosCandidatosSairam[key]) {
            dadosPorMaquina[maquinaId].produtosCandidatosSairam[key] = {
              produto: mp.produto,
            };
          }
        }
    ...
    const maquinasDetalhadas = Object.values(dadosPorMaquina).map((m) => {
      const totalProdutosSairamDetalhadoMaquina = Object.values(
        m.produtosSairam,
      ).reduce((sum, p) => sum + p.quantidade, 0);

      const totalProdutosEntraramDetalhadoMaquina = Object.values(
        m.produtosEntraram,
      ).reduce((sum, p) => sum + p.quantidade, 0);

      const produtosSairamMaquina = Object.values(m.produtosSairam);
      if (produtosSairamMaquina.length === 0 && m.totalSairam > 0) {
        const candidatos = Object.values(m.produtosCandidatosSairam);
        if (candidatos.length === 1) {
          produtosSairamMaquina.push({
            produto: candidatos[0].produto,
            quantidade: m.totalSairam,
          });
        }
      }

      const totalProdutosSairamExibicaoMaquina = produtosSairamMaquina.reduce(
        (sum, p) => sum + p.quantidade,
        0,
      );
      const produtosSairamParaCalculo =
        totalProdutosSairamExibicaoMaquina > 0
          ? totalProdutosSairamExibicaoMaquina
          : m.totalSairam;
      const faturamentoMaquina =
        m.valoresEntrada.fichas +
        m.valoresEntrada.notas +
        m.valoresEntrada.cartao;
      const custoPeluciasVendidas = produtosSairamMaquina.reduce((sum, p) => {
        const custoUnitarioCadastro = parseFloat(p.produto?.custoUnitario || 0);
        const valorUnitarioRelatorio = parseFloat(p.produto?.preco || 0);
        const custoUnitario =
          custoUnitarioCadastro > 0
            ? custoUnitarioCadastro
            : valorUnitarioRelatorio;
        return sum + custoUnitario * (p.quantidade || 0);
      }, 0);
      const lucroLiquidoMaquina = faturamentoMaquina - custoPeluciasVendidas;
      const mediaRecebidaPorPelucia =
        produtosSairamParaCalculo > 0
          ? lucroLiquidoMaquina / produtosSairamParaCalculo
          : 0;
      ...
      return {
        ...
        totais: {
          fichas: m.fichas,
          // Fallback: quando não há detalhamento por produto, usar total da movimentação.
          produtosSairam:
            totalProdutosSairamExibicaoMaquina > 0
              ? totalProdutosSairamExibicaoMaquina
              : m.totalSairam,
          produtosEntraram:
            totalProdutosEntraramDetalhadoMaquina > 0
              ? totalProdutosEntraramDetalhadoMaquina
              : m.totalAbastecidas,
          movimentacoes: m.numMovimentacoes,
          custoPeluciasVendidas,
          lucroLiquido: lucroLiquidoMaquina,
          mediaRecebidaPorPelucia,
        },
        valoresEntrada: {
          ...m.valoresEntrada,
          total: faturamentoMaquina,
        },
        indicadoresFinanceiros: {
          faturamento: faturamentoMaquina,
          custoPeluciasVendidas,
          lucroLiquido: lucroLiquidoMaquina,
          mediaRecebidaPorPelucia,
        },
        produtosSairam: produtosSairamMaquina
          .map((p) => ({
            ...
            custoUnitario: p.produto.custoUnitario,
            quantidade: p.quantidade,
          }))
    ...
    res.json({
      ...
      totais: {
        fichas: totalFichas,
        // Fallback: se não houve detalhamento por produto, usar total geral da movimentação.
        produtosSairam:
          totalProdutosSairamDetalhado > 0
            ? totalProdutosSairamDetalhado
            : totalSairam,
        produtosEntraram: totalAbastecidas,
        movimentacoes: movimentacoes.length,
        valoresEntrada: {
...
export const relatorioComissoes = async (req, res) => {
  try {
    ...
    comissoes.forEach((com) => {
      ...
      comissoesPorLoja[lojaNome].totalComissao += parseFloat(
        com.totalComissao || 0,
      );
